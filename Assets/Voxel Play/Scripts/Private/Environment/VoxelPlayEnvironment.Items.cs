...
namespace VoxelPlay {
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour {
        ...
        void InitItems () {
            ...
            LogMessage("Loading all item definitions in VoxelPlay/Defaults");
            ...
            LogMessage("Loading all item definitions in " + world.resourcesLocation);
            ids = Resources.LoadAll<ItemDefinition>(world.resourcesLocation);
            for (int k = 0; k < ids.Length; k++) {
                AddItemDefinition(ids[k]);
            }
        ...
        GameObject TorchAttachInt (VoxelHitInfo hitInfo, ItemDefinition torchDefinition = null, bool refreshChunks = true) {
            ...
            if (hitInfo.chunk.voxels[hitInfo.voxelIndex].isEmpty) return null;
            VoxelDefinition vd = voxelDefinitions[hitInfo.chunk.voxels[hitInfo.voxelIndex].typeIndex];
            if (!vd.renderType.isOpaque()) return null;

            // Torches can also be placed on half voxels or top half voxels
            bool isBottomHalf = false;
            bool isTopHalf = false;
            if (IsMicroVoxelAtPosition(hitInfo.chunk, hitInfo.voxelIndex, out MicroVoxels mv)) {
                isBottomHalf = mv.IsBottomHalf();
                isTopHalf = mv.IsTopHalf();
                if (!isBottomHalf && !isTopHalf) {
                    return null;
                }
            }
            ...
            // Adjust Y position if the voxel has half voxels
            if (isBottomHalf) {
                torchPos.y -= 0.25f;
            } else if (isTopHalf) {
                torchPos.y += 0.25f;
        ...
        GameObject CreateRecoverableItem (Vector3d position, ItemDefinition itemDefinition, float quantity = 1) {
