	...
	public partial class VoxelPlayEnvironment : MonoBehaviour {
		...
		void ReuseChunkEntry (Vector3d position) {
			...
			for (int i = 0; i < chunksPool.Length; i++) {
				...
				double dz = position.z - chunk.position.z;
		...
		/// <summary>
		/// Sets up a chunk from a pre-instantiated GameObject (used with InstantiateAsync)
		/// </summary>
		void SetupChunkPoolEntry (GameObject chunkGO, int poolIndex) {
			chunkGO.layer = layerVoxels;
			chunkGO.hideFlags = HideFlags.DontSave;
#if UNITY_EDITOR
			if (hideChunksInHierarchy) {
			...
			chunk.mr.enabled = false;
			chunk.mr.receiveShadows = enableShadows;
			chunk.mr.shadowCastingMode = enableShadows ? ShadowCastingMode.On : ShadowCastingMode.Off;
			chunk.mc = chunkGO.GetComponent<MeshCollider>();
			chunk.poolIndex = poolIndex;

			chunksPool[poolIndex] = chunk;
		}
