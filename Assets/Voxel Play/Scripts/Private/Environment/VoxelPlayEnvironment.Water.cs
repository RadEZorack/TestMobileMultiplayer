...
namespace VoxelPlay {
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour {
        ...
        /// <summary>
        /// Global flag to check if water can spread
        /// </summary>
        public bool canSpreadWater {
            get {
                if (!enableWaterFlood) return false;
                if (buildMode && !world.waterSpreadsInBuildMode) return false;
#if UNITY_EDITOR
                if (renderInEditor && !applicationIsPlaying) return false;
#endif
                return true;
            }
        }
        ...
        void MakeSurroundingWaterExpand (Vector3d position, int lifeTime = 24) {
            ...
            for (int k = 0; k < floodOffsetsCount; k += 3) {
                ...
                if (GetVoxelIndex(pos, out VoxelChunk otherChunk, out int otherVoxelIndex, false)) {
                    int otherWaterLevel = otherChunk.voxels[otherVoxelIndex].GetWaterLevel();
                    if (otherWaterLevel > 0) {
                        VoxelDefinition vd = voxelDefinitions[otherChunk.voxels[otherVoxelIndex].typeIndex];
                        if (vd.spreads) {
                            AddWaterFloodInt(ref pos, vd, lifeTime);
                        }
                    }
                }
            }
        ...
        void WaterUpdateLevelFast (VoxelChunk chunk, int voxelIndex, int waterLevel, VoxelDefinition vd) {
            ...
            if (waterLevel == 0) {
                ...
                VoxelDefinition existingVd = voxelDefinitions[chunk.voxels[voxelIndex].typeIndex];
                if (existingVd.index != vd.index) {
                    if (chunk.voxels[voxelIndex].typeIndex <= Voxel.HOLE_TYPE_INDEX || existingVd.renderType == RenderType.Water || existingVd.renderType == RenderType.Fluid || existingVd.renderType == RenderType.Cutout || existingVd.renderType == RenderType.CutoutCross || vd.spreadReplaceThreshold > chunk.voxels[voxelIndex].opaque) {
        ...
        void TryWaterSpread (Vector3d waterPos, int index) {
            ...
            if ((object)chunk == null || chunk.voxels[voxelIndex].typeIndex <= Voxel.HOLE_TYPE_INDEX) {
            ...
            if (GetVoxelIndex(down, out otherChunk, out vIndex)) {
                ...
                bool canFillWithWater = otherChunk.voxels[vIndex].typeIndex <= Voxel.HOLE_TYPE_INDEX || vdOther.renderType == RenderType.Cutout || vdOther.renderType == RenderType.CutoutCross || vdOther.renderType == RenderType.Water || vdOther.renderType == RenderType.Fluid;
                if (canFillWithWater) {
            ...
            for (int f = 0; f < floodHorizontalOffsets.Length; f += 2) {
                ...
                if (GetVoxelIndex(otherPos, out otherChunk, out vIndex, false)) {
                    VoxelDefinition vdOther = voxelDefinitions[otherChunk.voxels[vIndex].typeIndex];
                    if (otherChunk.voxels[vIndex].typeIndex <= Voxel.HOLE_TYPE_INDEX || vdOther.renderType == RenderType.Cutout || vdOther.renderType == RenderType.CutoutCross || vdOther.renderType == RenderType.Water || vdOther.renderType == RenderType.Fluid) {
                        int otherWaterLevel = otherChunk.voxels[vIndex].GetWaterLevel();
                        if (otherWaterLevel < waterLevel) {
                            leveling = 1;
                        }
                        // Allow equalization to same level as the current water voxel
                        if (otherWaterLevel < waterLevel) {
                            if (waterLevel > 1) {
            ...
            for (int f = 0; f < floodHorizontalOffsets.Length; f += 2) {
                ...
                if (GetVoxelIndex(otherPos, out otherChunk, out vIndex, false)) {
                    ...
                    if (otherWaterLevel > waterLevel) {
                        VoxelDefinition vdOther = voxelDefinitions[otherChunk.voxels[vIndex].typeIndex];
                        if (vdOther.index != waterVoxel.index) continue;
                        if (vdOther.drains) {
                            otherWaterLevel--;
                        ...
                        if (otherWaterLevel > waterLevel + leveling) {
                            if (OnVoxelBeforeSpread != null && !OnVoxelBeforeSpread(otherChunk.voxels[vIndex].type, waterPos, chunk.voxels[voxelIndex].type)) continue;
                            waterLevel++;
                            if (OnVoxelAfterSpread != null) {
                                OnVoxelAfterSpread(otherChunk.voxels[vIndex].type, waterPos);
                            }
                        }
                    }
                }
