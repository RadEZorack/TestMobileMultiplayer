...
namespace VoxelPlay {
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour {
        ...
        void ComputeSunLightmap(VoxelChunk chunk) {
            ...
            const int CHUNKS_SIZE_MINUS_ONE_ONE_Y_ROW = CHUNK_SIZE_MINUS_ONE * ONE_Y_ROW;
            const int CHUNKS_SIZE_MINUS_ONE_ONE_Z_ROW = CHUNK_SIZE_MINUS_ONE * ONE_Z_ROW;
            ...
            if (topChunkIsAccesible) {
                int top = CHUNKS_SIZE_MINUS_ONE_ONE_Y_ROW;
                for (int bottom = 0; bottom < ONE_Y_ROW; bottom++, top++) {
                ...
                for (int top = CHUNKS_SIZE_MINUS_ONE_ONE_Y_ROW; top < CHUNK_VOXEL_COUNT; top++) {
            ...
            if (leftChunkIsAccesible) {
                int left = CHUNKS_SIZE_MINUS_ONE_ONE_Y_ROW + CHUNK_SIZE_MINUS_ONE * ONE_Z_ROW;
                int right = left + CHUNK_SIZE_MINUS_ONE;
                for (int z = 0; z < ONE_Y_ROW; z++, right -= ONE_Z_ROW, left -= ONE_Z_ROW) {
            ...
            if (forwardChunkIsAccesible) {
                for (int y = CHUNK_SIZE_MINUS_ONE; y >= 0; y--) {
                    ...
                    int forward = back + CHUNKS_SIZE_MINUS_ONE_ONE_Z_ROW;
