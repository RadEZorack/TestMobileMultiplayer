...
namespace VoxelPlay {
    ...
    public partial class VoxelChunk : MonoBehaviour {
...
#if UNITY_EDITOR
        [Tooltip("Let you mark a chunk in SceneView and use this value for debugging purposes.")]
        public bool debug;
        [Tooltip("Useful when selecting a chunk in SceneView. Use this value for debugging purposes.")]
        ...
        /// <summary>
        /// The timestamp of the modification to be compared with the session stage
        /// </summary>
        ...
        /// <summary>
        /// Used when during meshing some neighbour is missing. When the neighbour populates, this chunk will need to be meshed again
        /// </summary>
        [NonSerialized]
        public bool missingLeftChunk, missingRightChunk, missingForwardChunk, missingBackChunk, missingTopChunk, missingBottomChunk;
        ...
        /// <summary>
        /// Clears voxels in a contiguous range of voxel indices
        /// </summary>
        /// <param name="startIndex">Starting voxel index (inclusive)</param>
        /// <param name="endIndex">Ending voxel index (inclusive)</param>
        /// <param name="light">Light intensity left at empty positions</param>
        public void ClearVoxels (int startIndex, int endIndex, byte light) {
            if (startIndex < 0) startIndex = 0;
            if (endIndex >= voxels.Length) endIndex = voxels.Length - 1;
            if (startIndex > endIndex) return;

            for (int voxelIndex = startIndex; voxelIndex <= endIndex; voxelIndex++) {
                if (voxels[voxelIndex].hasContent) {
                    if (lightSources != null) {
                        int lightSourcesCount = lightSources.Count;
                        for (int k = lightSourcesCount - 1; k >= 0; k--) {
                            LightSource ls = lightSources[k];
            ...
            // Mark chunk as needing refresh
            needsMeshRebuild = true;
            needsLightmapRebuild = true;
        }

        /// <summary>
        /// Clears voxels in a Y range within X/Z bounds
        /// </summary>
        /// <param name="startY">Starting Y level within chunk (0-15)</param>
        /// <param name="endY">Ending Y level within chunk (0-15, inclusive)</param>
        /// <param name="startX">Starting X coordinate within chunk (0-15)</param>
        /// <param name="endX">Ending X coordinate within chunk (0-15, inclusive)</param>
        /// <param name="startZ">Starting Z coordinate within chunk (0-15)</param>
        /// <param name="endZ">Ending Z coordinate within chunk (0-15, inclusive)</param>
        /// <param name="light">Light intensity left at empty positions</param>
        public void ClearVoxels (int startY, int endY, int startX, int endX, int startZ, int endZ, byte light) {
            if (startY < 0) startY = 0;
            if (endY >= VoxelPlayEnvironment.CHUNK_SIZE) endY = VoxelPlayEnvironment.CHUNK_SIZE - 1;
            ...
            for (int py = startY; py <= endY; py++) {
                int yBaseIndex = py * VoxelPlayEnvironment.ONE_Y_ROW;
                for (int pz = startZ; pz <= endZ; pz++) {
                    int zBaseIndex = yBaseIndex + pz * VoxelPlayEnvironment.ONE_Z_ROW;
                    for (int px = startX; px <= endX; px++) {
                        int voxelIndex = zBaseIndex + px;
                        if (voxels[voxelIndex].hasContent) {
                            if (lightSources != null) {
                                int lightSourcesCount = lightSources.Count;
                                for (int k = lightSourcesCount - 1; k >= 0; k--) {
        ...
        /// <summary>
        /// Returns true if this chunk has any voxel content (not fully empty)
        /// </summary>
        public bool HasContent () {
            if (voxels == null) return false;
            if (totalVisibleVoxelsCount > 0) return true;
            int count = voxels.Length;
            for (int i = 0; i < count; i++) {
                if (voxels[i].hasContent) return true;
            }
            return false;
        }
        ...
        void OnDestroy () {
            ...
            if (voxelsExtraData != null) {
                voxelsExtraData.Clear();
                voxelsExtraData = null;
            }
            if (voxelsProperties != null) {
                voxelsProperties.Clear();
                voxelsProperties = null;
            }
            if (items != null) {
                items.Clear();
                items = null;
        ...
        public void SetVoxel (int voxelIndex, VoxelDefinition voxelDefinition) {
            SetVoxel(voxelIndex, voxelDefinition, voxelDefinition.tintColor);
        }
        ...
        public void SetVoxel (int voxelIndex, VoxelDefinition voxelDefinition, Color32 tintColor) {
            ...
            SetMicroVoxels(voxelIndex, voxelDefinition.microVoxels);
        }

        /// <summary>
        /// Replaces all voxels in this chunk with the given voxels array
        /// </summary>
        /// <param name="voxels">Voxels array to set</param>
