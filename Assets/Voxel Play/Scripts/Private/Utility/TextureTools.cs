...
namespace VoxelPlay {
    ...
    public static class TextureTools {
        ...
        public static void ReleaseCache (Texture2D tex) {
            if (tex != null) {
                scaledTextures.Remove(tex);
            }
        }

        public static Color32[] ScaleTextureColors (Texture2D tex, int width, int height, FilterMode mode = FilterMode.Trilinear, bool canUseCache = true) {

            if (canUseCache && scaledTextures.TryGetValue(tex, out Color32[] scaledColors) && scaledColors.Length == width * height) return scaledColors;
