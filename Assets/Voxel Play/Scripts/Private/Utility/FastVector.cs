...
namespace VoxelPlay {
	public static class FastVector {
		...
		public static void ExpandBounds(ref Vector3 boundsMin, ref Vector3 boundsMax, Vector3 position, Vector3 size) {
			if (position.x - size.x < boundsMin.x) boundsMin.x = position.x - size.x;
			if (position.y - size.y < boundsMin.y) boundsMin.y = position.y - size.y;
			if (position.z - size.z < boundsMin.z) boundsMin.z = position.z - size.z;
			if (position.x + size.x > boundsMax.x) boundsMax.x = position.x + size.x;
			if (position.y + size.y > boundsMax.y) boundsMax.y = position.y + size.y;
			if (position.z + size.z > boundsMax.z) boundsMax.z = position.z + size.z;
		}
