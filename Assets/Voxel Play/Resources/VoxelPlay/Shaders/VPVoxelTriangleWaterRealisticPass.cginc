...
//    half3 normal = (bump1 + bump2).xzy * 0.5;
    ...
    if (Linear01DepthX(depth) > 0.999 && _VPWaterDepthClip < 1) return 0;
    ...
    // Specular
    float3 uwv = worldViewDir;
    uwv.y = abs(uwv.y); // specular underwater
    float3 h = normalize (_WorldSpaceLightPos0.xyz + uwv);
    ...
    half3 foamColor = tex2D(_FoamTex, i.bumpuv.xy * 7.0 + foamDistortUV).rgb * _FoamColor.rgb;
