...
namespace VoxelPlay
{
	...
	public class TorchLightAnimator : MonoBehaviour
	{
		...
		const float SparkTimeStepPerFrame = 0.1f;
		...
		static readonly int TorchMainTexOffsetId = Shader.PropertyToID("_TorchMainTexOffset");
		...
		void Start ()
		{
			...
			if (meshRenderer != null) {
				materialPropertyBlock.SetFloat (TorchMainTexOffsetId, mainTexOffset);
		...
		void Update ()
		{
			...
			if (animateParticles) {
				...
				float frameFactor = deltaTime * ReferenceFrameRate;

				if (meshRenderer != null) {
					mainTexOffset -= deltaTime * 2f;
					materialPropertyBlock.SetFloat (TorchMainTexOffsetId, mainTexOffset);
					meshRenderer.SetPropertyBlock (materialPropertyBlock);
				}
				...
				for (int k = 0; k < sparks.Length; k++) {
					sparksTime[k] += SparkTimeStepPerFrame * frameFactor;
					if (sparksTime[k] > sparkLiftTime) {
						...
						Vector3 delta;
						delta.x = localScale.x * (Mathf.Sin(tr) * sparksTime[k]) * sparkSpread * frameFactor;
						delta.z = localScale.y * (Mathf.Cos(tr) * sparksTime[k]) * sparkSpread * frameFactor;
